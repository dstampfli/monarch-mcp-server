...
- **Cookie input avoids the terminal, not just for convenience.** `login_setup.py`'s cookie path (`_read_cookie_string`) reads `MONARCH_COOKIE` (loaded from a gitignored `.env` via `python-dotenv`) or a file path, and only falls back to a `getpass` paste. This is deliberate: macOS canonical-mode input is capped at `MAX_CANON` (1024 bytes), and a real Monarch `cookie:` header exceeds that, so pasting silently truncates it. Don't "simplify" the cookie read back to a plain prompt. `.env.example` documents the variables.
